...
(async () => {
  ...
  assert(/claims\.sub !== userId/.test(edge), 'MCP server must bind the JWT subject to the verified Auth user');
  assert(!/claims\.user_id/.test(edge), 'MCP server must not require the access-token hook event user_id as a JWT claim');
